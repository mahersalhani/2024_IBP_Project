  ...
  return (
    <>
      <MenuItem>
        <button className="flex w-full text-2xl" onClick={() => setSubMenuOpen(!subMenuOpen)}>
          <div className="flex flex-row justify-between w-full items-center">
            <span className={`${pathname.includes(item.path) ? 'font-bold' : ''}`}>{t(item.title)}</span>
            <div className={`${subMenuOpen && 'rotate-180'} transform transition-transforms`}>
      ...
      <motion.div
        ...
        className="mt-2 ml-2 flex flex-col space-y-2 overflow-hidden"
      >
