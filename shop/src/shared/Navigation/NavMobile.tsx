...
const NavMobile: React.FC<NavMobileProps> = ({ data = NAVIGATION_DEMO_2, onClickClose }) => {
      ...
      <ul className="nav-mobile-sub-menu pl-6 pb-1 text-base">
        {item.children?.map((i, index) => (
          <Disclosure key={index} as="li">
            ...
            {i.children && (
              <Disclosure.Panel>{_renderMenuChild(i, 'pl-3 text-slate-600 dark:text-slate-400 ')}</Disclosure.Panel>
            )}
          </Disclosure>
        ))}
      </ul>
  ...
  const _renderItem = (item: NavItemType, index: number) => {
    if (item.onlyAdmin && user?.accountType !== 'admin') return null;
    if (item.onlyCustomer && user?.accountType !== 'customer') return null;
